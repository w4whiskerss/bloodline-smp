package dev.whiskers.bloodline.listener;
...
public final class BloodlineListener implements Listener {
    ...
    public void onHeldItemChange(PlayerItemHeldEvent event) {
        ...
        if (manager.isBloodlineGameplayDisabled(player) || manager.usesClientHotkeys(player) || !player.isSneaking()) {
            manager.pushClientState(player, false);
            return;
        }
        ...
        switch (event.getNewSlot()) {
            ...
            case 1 -> {
                event.setCancelled(true);
                manager.triggerFourth(player);
            }
            case 2 -> {
                event.setCancelled(true);
                manager.triggerFifth(player);
            }
            default -> manager.pushClientState(player, false);
        }
    }
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (manager.isDomainWand(item) && event.getClickedBlock() != null) {
            event.setCancelled(true);
            if (action == Action.LEFT_CLICK_BLOCK) {
                manager.selectDomainPoint(player, event.getClickedBlock().getLocation(), true);
            } else if (action == Action.RIGHT_CLICK_BLOCK) {
                manager.selectDomainPoint(player, event.getClickedBlock().getLocation(), false);
            }
            return;
        }
        ...
        if (plugin.getCustomItems().isBloodMace(item)) {
            ...
            if (player.isSneaking()) {
                manager.triggerBloodMaceSecondary(player);
            } else if (action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK) {
                manager.triggerBloodMacePrimary(player);
            }
            return;
        }
    ...
    public void onMove(PlayerMoveEvent event) {
        if (manager.isBloodMaceFrozen(event.getPlayer()) && event.getTo() != null && event.getFrom().distanceSquared(event.getTo()) > 0.0001D) {
    ...
    public void onDamage(EntityDamageByEntityEvent event) {
        ...
        manager.amplifyBloodMaceFrozenDamage(event);
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        if (event.getEntity() instanceof Player || event.getEntity().getKiller() == null || !manager.canDropBlood()) {
            return;
        }
        if (Math.random() < plugin.getConfig().getDouble("drops.mob-blood-drop-chance", 0.025D)) {
            event.getDrops().add(plugin.getCustomItems().createBloodDrop());
        }
    }

    @EventHandler(ignoreCancelled = true)
    public void onConsume(PlayerItemConsumeEvent event) {
        if (manager.isBloodMaceFrozen(event.getPlayer())) {
            event.setCancelled(true);
            return;
        }
    ...
    public void onCraft(CraftItemEvent event) {
        ...
        ItemStack current = event.getCurrentItem();
        if (expected != null && plugin.getCustomItems().isBloodMace(expected)) {
            if (!(event.getWhoClicked() instanceof Player player) || event.isShiftClick()) {
                event.setCancelled(true);
                return;
            }
            ItemStack crafted = manager.finalizeBloodMaceCraft(player);
            if (crafted == null) {
                event.setCancelled(true);
                return;
            }
